...
export function DocumentosForm({ dados, onChange, fieldConfig }: Props) {
  const handleChange = (field: keyof PreCadastro, value: string) => {
    onChange({ ...dados, [field]: value });
  };

  const isEnabled = (key: string) => fieldConfig ? fieldConfig.isFieldEnabled(key) : true;
