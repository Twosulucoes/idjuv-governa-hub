/**
 * DEV MODE SWITCHER
 * Componente para alternar entre módulos em tempo real para testes.
 */

import { useState, useEffect } from 'react';
import { Settings2, ShieldCheck, ChevronDown, ChevronUp, X, AlertTriangle } from 'lucide-react';
...
export function DevModeSwitcher() {
  ...
  const saveOverrides = (modules: Modulo[], superAdmin: boolean) => {
    const overrides: DevModeOverrides = { modules, isSuperAdmin: superAdmin };
  ...
  const handleSuperAdminToggle = (checked: boolean) => {
    ...
    if (checked) {
      ...
      saveOverrides(selectedModules, false);
    }
  };
  ...
  const handleModuleToggle = (modulo: Modulo) => {
    ...
    saveOverrides(newModules, isSuperAdmin);
  };
    ...
    <>
      {isEnabled && (
        <div className="fixed top-0 left-0 right-0 z-[100] bg-amber-500 text-amber-950 py-1.5 px-4 text-sm font-medium flex items-center justify-center gap-2 shadow-md">
          ...
          <span>
            Modo Simulação: <strong>{isSuperAdmin ? 'Super Admin' : `${selectedModules.length} módulo(s)`}</strong>
          </span>
          <Button
            variant="ghost"
            size="sm"
      ...
      <Button
        ...
        className={`fixed bottom-4 right-4 z-50 rounded-full w-12 h-12 shadow-lg bg-background border-2 ${
          isEnabled ? 'border-amber-500 hover:border-amber-600' : 'border-primary/50 hover:border-primary'
        }`}
        ...
        <Card className="fixed bottom-20 right-4 z-50 w-80 shadow-xl animate-in slide-in-from-bottom-5">
          <CardHeader className="pb-3">
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              <Switch id="dev-mode-toggle" checked={isEnabled} onCheckedChange={handleEnableToggle} />
            </div>
          </CardHeader>
            ...
            <CardContent className="space-y-4 pt-0">
              <div className="flex items-center justify-between p-2 rounded-lg bg-muted/50">
                <div className="flex items-center gap-2">
                  ...
                  <Label htmlFor="super-admin" className="text-sm font-medium">Super Admin</Label>
                </div>
                <Switch id="super-admin" checked={isSuperAdmin} onCheckedChange={handleSuperAdminToggle} />
              </div>

              <Collapsible open={isExpanded} onOpenChange={setIsExpanded}>
                <CollapsibleTrigger asChild>
                  <Button variant="ghost" size="sm" className="w-full justify-between">
                    <span className="text-xs">Módulos ({selectedModules.length}/{MODULOS.length})</span>
                  ...
                  <div className="grid grid-cols-1 gap-1 max-h-48 overflow-y-auto">
                    {MODULES_CONFIG.map((config) => (
                      ...
                      >
                        ...
                        <Label htmlFor={`mod-${config.codigo}`} className="text-xs cursor-pointer flex-1">
                          {config.nome}
                        </Label>
                      </div>
                    ))}
              ...
              <div className="pt-2 border-t">
                ...
                <div className="flex flex-wrap gap-1">
                  {isSuperAdmin && <Badge variant="default" className="text-xs bg-amber-600">Super Admin</Badge>}
