...
const contatosPadrao: Contato[] = [
  {
    ...
    valor: "https://www.cob.org.br/pt/jogos-da-juventude",
    icone: "trophy",
    ordem: 1,
  },
  {
...
export function SeletivaContatosV2() {
  const { data: contatos } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("contatos_eventos_esportivos")
        .select("*")
        .eq("ativo", true)
        .eq("evento", "seletivas_2026")
  ...
  return (
    <section className="py-16 px-4 bg-zinc-900 dark:bg-zinc-950 transition-colors">
      <div className="container mx-auto max-w-6xl">
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      {site.subtitulo && (
                        <p className="text-sm text-zinc-400">{site.subtitulo}</p>
                      )}
                    </div>
        ...
        {contatosDiretos.length > 0 && (
          ...
          >
            {contatosDiretos.map((contato, index) => {
              ...
              const Wrapper = isLink(contato.valor) || isEmail(contato.valor) || isPhone(contato.valor)
                ? "a"
                : "div";

              const wrapperProps = Wrapper === "a"
                ? { href, target: isLink(contato.valor) ? "_blank" : undefined, rel: isLink(contato.valor) ? "noopener noreferrer" : undefined }
                : {};
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {contato.subtitulo && (
                        <p className="text-xs text-zinc-500 mt-0.5">{contato.subtitulo}</p>
                      )}
        ...
        >
          <p className="text-xs text-zinc-500 max-w-xl mx-auto">
            Os Jogos da Juventude são organizados pelo Comitê Olímpico do Brasil (COB). 
            Os Jogos Escolares Brasileiros são de responsabilidade da CBDE.
          </p>
        </motion.div>
      </div>
    </section>
