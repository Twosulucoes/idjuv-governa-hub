-- Corrigir função processar_folha_pagamento com nomes corretos das colunas
-- p.ativo = true -> p.status = 'ativo'
-- s.situacao_funcional = 'ativo' -> s.situacao = 'ativo'
...
BEGIN
  ...
  -- CORREÇÃO: usar p.status = 'ativo' e s.situacao = 'ativo'
  FOR v_servidor IN
    SELECT 
      s.id,
      s.nome_completo,
